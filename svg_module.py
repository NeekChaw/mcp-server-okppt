...
def insert_svg_to_pptx(
    ...
    **实现方式:**
    ...
    Args:
        ...
        create_if_not_exists: 如果为True且输入文件不存在，将创建一个新的PPTX文件。

    Returns:
        bool: 如果成功插入则返回 True，否则返回 False。

    Raises:
        FileNotFoundError: 如果 pptx_path 无效且 create_if_not_exists 为 False。
    ...
    try:
            ...
            try:
                ...
                if sldSz is not None:
                    ...
                    if not default_width_emu or not default_height_emu:
                        print("Warning: Could not read valid cx or cy from presentation.xml. Default size might be incorrect.")
                        default_width_emu = default_height_emu = None # Reset if invalid
                else:
                    print("Warning: <p:sldSz> element not found in presentation.xml. Cannot determine default slide size.")
            except etree.XMLSyntaxError as e:
                print(f"Warning: Could not parse presentation.xml: {e}. Cannot determine default slide size.")
        else:
            print("Warning: presentation.xml not found. Cannot determine default slide size.")
        ...
        if default_width_emu is None or default_height_emu is None:
            ...
            print(f"Warning: Using fallback default size: width={default_width_emu}, height={default_height_emu} EMU.")
        ...
        try:
            ...
            print(f"Error converting SVG to PNG using reportlab/svglib: {e}")
        ...
        if height is None:
            if width is None: # 完全默认，使用幻灯片高度
                ...
                if svg_width_px and svg_height_px and svg_width_px > 0 and svg_height_px > 0:
                    ...
                    print(f"Info: Calculated height based on SVG aspect ratio: {height_emu} EMU")
                else:
                    print(f"Warning: Could not determine SVG aspect ratio. Using default height: {default_height_emu} EMU")
                    height_emu = default_height_emu
        else: # 用户指定了高度
        ...
        if not os.path.exists(rels_path):
             print(f"Error: Relationship file not found for slide {slide_number}: {rels_path}")
             if os.path.exists(png_target_path): os.remove(png_target_path)
             if os.path.exists(svg_target_path): os.remove(svg_target_path)
             return False
        ...
        if spTree is None:
            ...
            if cSld is None:
                 print(f"Error: Could not find <p:cSld> in slide {slide_number}.")
                 if os.path.exists(png_target_path): os.remove(png_target_path)
                 if os.path.exists(svg_target_path): os.remove(svg_target_path)
                 return False
        ...
        if not os.path.exists(content_types_path):
            print(f"Error: [Content_Types].xml not found at {content_types_path}")
            if os.path.exists(png_target_path): os.remove(png_target_path)
            if os.path.exists(svg_target_path): os.remove(svg_target_path)
            return False
